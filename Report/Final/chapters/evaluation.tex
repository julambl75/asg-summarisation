...
The \textit{encoder} and \textit{decoder} share embeddings for a vocabulary of size 7295, which is internally represented using a vector of size 100.

On the encoder side, we use a two layer LSTM with a \textit{dropout} of 0.4. The decoder side is the same, except that \textit{global attention} is applied at the end.

\subsection{Training}

We train our neural network using an Adam optimizer and a \textit{learning rate} of 0.0005. This is done over a period of 10,000 epochs, running on test data every 100 epochs.
