...
In the case of more complex \textit{noun phrases} we use a special predicate \texttt{conjunct(\underline{first},\underline{second})}, allowing us to join two words of the same \textit{role} together. Here, we also need to add a constraint that rules out cases where the two \textit{conjuncts} have the same lemma.

For example, the \textit{noun phrase} ``green and blue" would be encompassed by the below derivation:
