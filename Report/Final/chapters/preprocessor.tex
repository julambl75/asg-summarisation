...
This is what we call \textit{story homogenization}, and it helps \textsc{SumASG} link words that would otherwise be considered completely different tokens in the story.

\subsection{Example}

An example is shown below in Figure \ref{fig:text_relationship_map_example} for the story of Peter Little, where the weight between two nodes refers to \textit{similarity}. In this example, we consider words to be synonyms whenever their \textit{similarity} is at least 20. The labels of the nodes (starting from 0) for sentence \textit{similarity} correspond to the index of the sentence in the simplified story (see Subsection \ref{subsec:simplification_example}). As you can see, sentence 5 is deemed irrelevant and will be pruned, while sentence 0 is very important.
...
\end{subfigure}
\caption{Text relationship map for the example of Peter Little}
\label{fig:text_relationship_map_example}
\end{figure}
