...
\node (story_out) [inter, right =of homogenise] {Preprocessed Story};
...
It iterates over each sentence, and compares each word with every word from other sentences which have the same or a related POS tag. For each comparison, word \textit{similarity} is computed using the tool \textbf{\href{http://www.conceptnet.io}{ConceptNet}}, a semantic knowledge network.
